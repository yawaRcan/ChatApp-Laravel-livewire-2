<div>
    <div class="intro-y chat-input box border-theme-3 dark:bg-dark-2 dark:border-dark-2 border flex items-center px-5 py-4">
        ...
        <textarea class="form-control h-12 shadow-none resize-none border-transparent px-5 py-3 focus:shadow-none truncate mr-3 sm:mr-0" rows="1" placeholder="Type your message..." wire:model="message" id ="textArea"></textarea>
        <div class="dropdown relative mr-3 sm:mr-5" data-placement="top-end" wire:ignore>
                ...
                <div class="dropdown-menu__content box dark:text-gray-300 dark:bg-dark-3 shadow-md">
                    <div class="chat-input__smiley__box flex flex-col pb-3 -mt-2">
                        ...
                        <div class="tab-content overflow-hidden mt-5">
                            {{-- Empjies components --}}
                            @livewire('components.emojies', key('emojies'))
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <button class="bg-theme-1 text-white w-8 h-8 sm:w-10 sm:h-10 block rounded-full flex-none flex items-center justify-center" wire:click="storeMessage();">  
...
<script>
  $(function() {
    $(".rounded").on('click', function() {
      ...
      if (val !== '') {
        let oldEmojis = @this.get('message');
        let newEmojis = oldEmojis + ' ' + val;
        @this.set('message', newEmojis);
      }
    });
  });
</script>
